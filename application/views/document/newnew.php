...
<div class="container-fluid mt--6">
  <div class="row">
        ...
        <div class="col-lg-4">
                ...
                <div class="col-lg-12">
                    ...
                    <!-- <button class="btn btn-icon btn-info rounded-pill mb-3" id="ulangiSignature" type="button">
                        ...
                        <button class="btn btn-icon btn-default rounded-pill mb-3" id="printSignature" type="button">
                            <span class="btn-inner--icon"><i class="fas fa-print"></i></span>
                            <span class="btn-inner--text">Unduh</span>
                        </button>
                        <button class="btn btn-icon btn-success rounded-pill mb-3" id="simpanSignature" name="simpanSignature" type="button">
    ...
    // Handle page navigation
        ...
        document.getElementById('printSignature').addEventListener('click', async function() {
            var iduser              = document.getElementById("iduser").value;
            var iddocument          = document.getElementById("iddocument").value;
            ...
            const totalPagesBefore = pages.length; // Simpan jumlah halaman awal
            ...
            // **Deteksi apakah halaman terakhir adalah Audit Tracker**
            const auditTrackerText = "AUDIT TRACKER";
            const isLastPageAuditTracker = lastPageIndex >= 0 && lastPage.getSize && totalPagesBefore > 1;

            // Jika halaman terakhir adalah Audit Tracker, hapus sebelum menambahkan yang baru
            if (isLastPageAuditTracker) {
                pdfDoc.removePage(lastPageIndex);
            }

            // **Tambahkan halaman baru untuk Audit Tracker**
            const signPageDownload = pdfDoc.addPage([docWidth, docHeight]);

            // **Tambahkan teks AUDIT TRACKER di halaman baru**
            const { width: pageWidth, height: pageHeight } = signPageDownload.getSize();
            signPageDownload.drawText(auditTrackerText, { 
                x: pageWidth / 2 - 50, 
                y: pageHeight - 50, 
                size: 20, 
                color: PDFLib.rgb(0, 0, 0) 
            });
            signPageDownload.drawText(`Dokumen Ditandatangani oleh:`, { x: 50, y: pageHeight - 80, size: 14 });
                ...
                for (const approval of approvals) {
                    const { no_signature, nama, signature_photo, updated_date, status_signature } = approval;

                    // Konversi status_signature ke angka
                    console.log(status_signature);

                    // Konversi status_signature ke angka
                    const statusSignatureNum = parseInt(status_signature, 10);
                    console.log("Status Signature (After Conversion):", statusSignatureNum);

                    if (!updated_date || updated_date === "0000-00-00 00:00:00") {
                        console.warn(`Tanggal tidak valid, dilewati: ${updated_date}`);
                        formattedDate = "-"; // Gunakan teks default, bukan continue
                    } else {
                        // Pastikan format tanggal sesuai untuk JavaScript
                        let formattedDateStr = updated_date.replace(" ", "T");
                        let dateObj = new Date(formattedDateStr);

                        // Cek apakah hasil parsing valid
                        if (isNaN(dateObj.getTime())) {
                            console.error(`Gagal mengonversi tanggal: ${updated_date}`);
                            formattedDate = "Tanggal tidak tersedia"; // Gunakan teks default
                        } else {
                            ...
                            formattedDate = formatter.format(dateObj).replace(" pukul", "");
                        }
                    }
                    if (statusSignatureNum === 1) {
                        console.log("cek1")
                            // Tambahkan teks tanda tangan
                            signPageDownload.drawText(`Level tanda tangan ${no_signature} oleh: ${nama} status -`, { 
                                x: 50, 
                                y: yOffset, 
                        ...
                        } else if(statusSignatureNum === 2) {
                            console.log("cek2")
                            // Tambahkan teks tanda tangan
                            signPageDownload.drawText(`Level tanda tangan ${no_signature} oleh: ${nama} approve tanggal ${formattedDate}`, { 
                                x: 50, 
                                y: yOffset, 
                                size: 10,  // Ukuran font lebih kecil
                                color: PDFLib.rgb(0, 0, 0) 
                            ...
                            signPageDownload.drawImage(signatureImage, {
                            ...
                            console.log("cek3")
                            // Tambahkan teks tanda tangan
                            signPageDownload.drawText(`Level tanda tangan ${no_signature} oleh: ${nama} reject tanggal ${formattedDate}`, { 
                                x: 50, 
                                y: yOffset, 
                                size: 10,  // Ukuran font lebih kecil
                                color: PDFLib.rgb(0, 0, 0) 
                ...
                a.href = blobUrl;
                a.download = `${newFileName}`; // Nama file yang diunduh
                document.body.appendChild(a);
                a.click();
                document.body.removeChild(a);

                // Opsional: Hapus URL blob setelah digunakan
                URL.revokeObjectURL(blobUrl);
         
        });

        document.getElementById('simpanSignature').addEventListener('click', function() {
            bukaSwalDanProses();
        });
            ...
            const { value: textketerangan } = await Swal.fire({
                    ...
                    willOpen: () => {
                        setTimeout(() => {
                            ...
                            if (textarea) {
                                textarea.focus(); // Fokus manual setelah delay
                            }
                        }, 300); // Delay kecil biar modal fully loaded
                    }
                });
        ...
        if (textketerangan !== undefined) { // Jika pengguna menekan Submit
           ...
           await new Promise(resolve => setTimeout(resolve, 500));
            ...
            if (statusapprovalsign == "last") {
                ...
                try {
                    const extractedText = await lastPage.getTextContent(); // Ambil teks halaman terakhir
                ...
                signPage.drawText(`Dokumen Ditandatangani oleh:`, { x: 50, y: pageHeight - 80, size: 14 });
                ...
                // **Loop semua tanda tangan dari database**
                for (const approval of approvals) {
                    let { no_signature, nama, signature_photo, updated_date, status_signature } = approval;

                       // Cek jika ini adalah iterasi terakhir
                    if (approval === lastApproval) {
                        status_signature = "2"; // Ubah status menjadi 2
                        updated_date = new Date(); // Set updated_date ke waktu sekarang
                    }


                    // Konversi status_signature ke angka
                    const status = parseInt(status_signature, 10); 
                    

                    // Format tanggal menjadi "11 Maret 2025 21:15"
                    const displayDate = new Date(updated_date);
                    ...
                    if (status_signature == 1) {
                             // Tambahkan teks tanda tangan
                             signPage.drawText(`Level tanda tangan ${no_signature} oleh: ${nama} status -`, { 
                                x: 50, 
                                y: yOffset, 
                                size: 10,  // Ukuran font lebih kecil
                                color: PDFLib.rgb(0, 0, 0) 
                        ...
                        signPage.drawImage(signatureImage, {
                          ...
                          // Tambahkan teks tanda tangan
                          signPage.drawText(`Level tanda tangan ${no_signature} oleh: ${nama} reject tanggal ${formattedDate}`, { 
                                x: 50, 
                                y: yOffset, 
                                size: 10,  // Ukuran font lebih kecil
                                color: PDFLib.rgb(0, 0, 0) 
