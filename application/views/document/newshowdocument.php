  ...
  <div class="row">
    ...
    // Handle page navigation
        ...
        document.getElementById('simpanSignature').addEventListener('click', async function() {
            // Menampilkan SweetAlert dengan input text untuk keterangan
            const { value: textketerangan } = await Swal.fire({
                title: 'Alasan Menyetujuinya Apa',
                input: 'textarea',
                ...
                inputValidator: (value) => {
                    return null; // atau bisa pakai validasi sebenarnya
                },
                allowOutsideClick: () => !Swal.isLoading(), // mencegah klik luar menutup popup
                allowEscapeKey: true,
                allowOutsideClick: false,
        ...
        if (textketerangan !== undefined) { // Jika pengguna menekan Submit
           ...
           await new Promise(resolve => setTimeout(resolve, 500));
            ...
            const activePage = pdfDoc.getPage(pdfDoc.getPageCount() - 1); // Ambil halaman terakhir
            ...
            if(statusapprovalsign == "last") {
                ...
                activePage.drawImage(qrCodeImage, {
                    x: activePage.getWidth() - qrCodeWidth - 10,
                    y: 10,
                    width: qrCodeWidth,
                    height: qrCodeHeight,
                });
