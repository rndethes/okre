...
ini_set('display_errors', 1);
  ...
  class Project extends CI_Controller
  {
    ...
    public function index()
    {
     ...
     //  $data['checkmyid']       = $this->Space_model->checkAnggotaSpace($iduser);
    ...
    public function projectAtWorkspace($idspace){
      ...
      $data['spaceteam']      = $this->Space_model->checkUserAtTeam($idspace);
      ...
      $data['total_document']      = $this->Project_model->countAllProjectinsSpace();
      $data['all_project']         = $this->Project_model->countAllProjectinsSpace();
    ...
    public function getData()
    {
      ...
      foreach ($result as $result) {
        ...
        $url = base_url('project/projectAll/') . $idspace;
      
        $row[] = '
        <a href="' . base_url('project/showOkr/') . $result->id_project . '" onclick="saveCurrentUrl("'.$url.'");" class="btn btn-default btn-sm rounded-pill font-btn"> 
    ...
    public function getDataInWorkSpace()
    {
      ...
      foreach ($result as $result) {
        ...
        if($can_edit == '1') {
          $href = base_url('project/editProjectAll/') . $result->id_project;
          $class = 'btn-warning';
        } else {
          $href = "";
          $class = 'btn-secondary';
        ...
        if($can_delete == '1') {
          $idhapus = 'tombol-hapus';
          $classdel = 'btn-danger';
        } else {
          $idhapus = "";
          $classdel = 'btn-secondary';
        }


        $url = base_url('project/projectAll/') . $id_space;
      
          $row[] = '
              <a href="' . base_url('project/showOkr/') . $result->id_project . '" onclick="saveCurrentUrl('."'". base_url() .'project/projectAll/'. $id_space ."'".');" class="btn btn-default btn-sm rounded-pill font-btn"> 
              <span class="btn--inner-icon">
                <i class="ni ni-chart-bar-32"></i></span>
              <span class="btn-inner--text">Masuk OKR</span>
            </a>
          <a href="' . $href . '" class="btn '.$class.' btn-sm rounded-pill" onclick="saveCurrentUrl('."'". base_url() .'project/projectAll/'. $id_space ."'".');">
                              <span class="btn--inner-icon">
                                <i class="ni ni-settings"></i></span>
                              <span class="btn-inner--text"></span>
                            </a>
                            <a data-target="' . base_url('project/deleteProjectAll/') . $result->id_project .'/'.$result->id_team. '/' . $id_space .'" class="btn '.$classdel.' btn-sm '.$idhapus.' rounded-pill text-white" >
                              <span class="btn--inner-icon">
                                <i class="fas fa-trash"></i></span>
                              <span class="btn-inner--text"></span>
                            </a>';
    ...
    if( $priority_project = '' ) {
      ...
      $chatroom = array(
        'id_space_rc'       => $idspace,
        'id_project_rc'     => $generateIDProject,
        'name_mcr'          => $nama_project,
        'created_at_mcr'    => date("Y-m-d H:i:s"),
        'created_by_mcr	'   => $this->session->userdata("id"),
    );

    $this->Space_model->insert_roommessage($chatroom);


      $cekteamspace = $this->Team_model->cekTeamSpace($idspace);

      foreach($cekteamspace as $cteam) {

        if($cteam['id_user'] == $userid) {
          $acc_team = array(
            'id_team' => $id_team,
            'id_user'  => $cteam['id_user'],
            'role_user' => 'admin',
            'can_edit_okr' => 1,
            'can_delete_okr' => 1,
          );
    ...
    public function inputOkr()
    {
      ...
      $checkAccTeam = $this->Team_model->checkAccTeamInObj($id_team);

      foreach($checkAccTeam as $tm) {
        if($myid == $tm['id_user']) {
          $data = array(
            'id_objective'            => $id_okr,
            'id_user'                 => $tm['id_user'],
          ...
          $data = array(
            ...
            'role_user'               => 'editor',
            'can_edit_objective'      => 1,
            'can_delete_objective'    => 0,
          );
        }
        $accobj = $this->Project_model->input_okr($data, 'access_objective');
      }
    ...
    public function deleteOkr()
    {
      ...
      // Cek apakah $sum_value atau $total_precen adalah 0
      if ($total_precen == 0 || $sum_value == 0) {
        $value_project = 0;
      } else {
        $value_project = round(($sum_value / $total_precen) * 100, 2);
      }
    ...
    public function showKey($id_project, $id)
    {
      ...
      $data['user']  = $this->Team_model->getUserActiveInSpace($sesiworkspace);
      ...
      $documentsign       = $this->Space_model->dataMyDocumentInSpace($iduser,$sesiworkspace);
        $documentcreated    = $this->Space_model->dataMyDocumentAllInSpace($iduser,$sesiworkspace);
    ...
    public function inputInisiative($id_pjkr, $id_okr)
    {
      ...
      $start_date         = $this->input->post('dateawal[]');
      $due_date           = $this->input->post('dateakhir[]');
    ...
    public function getDetail()
    {
      ...
      $checkmyakses = checkMyAksesKey($idokr);
      ...
      foreach ($data as $data) {
       ...
       $checkdelegate      = $this->Project_model->checkDelegateInisiativeRise($id_initiative);
        ...
        $checktask  = $this->Space_model->checkTaskInOkr($id_initiative);
        $checkdoc   = $this->Space_model->checkOkrInDocument($id_initiative);

        if($checktask) {
          $task = '<span class="badge badge-primary">Terhubung Ke Task</span>';
        } else {
          $task = '';
        }

        if($checkdoc) {
          $doc = '<span class="badge badge-warning">Terhubung Ke Doc</span>';
        } else {
          $doc = '';
        }
        ...
        if($data['start_dateinit'] == NULL || $data['start_dateinit'] == "0000-00-00 00:00:00") {
          $startdate = "0-0";
        } else {
          $startdate = date("j F Y",strtotime($data['start_dateinit']));
        }

        if($data['due_dateinit'] == NULL || $data['start_dateinit'] == "0000-00-00 00:00:00") {
          $duedate = "0-0";
        } else {
          $duedate = date("j F Y",strtotime($data['due_dateinit']));
        }

       
        if($can_edit == '1') {
          $targetmodal = "#showEditModal";
          $btnedit = 'btn-warning text-white';
        } else {
          $targetmodal = "#notActive";
          $btnedit = 'btn-secondary text-grey';
        }
        ...
        if($can_delete == '1'){
          ...
          $class = "";
          $btndel = 'btn-secondary text-grey';
        }

        $detail = '<tr>
          <td>' . $data['description'] . '  ' . $task. ' ' .$doc.'</td>  
          ...
          <span class="btn-inner--text">Show</span>
           ...
           <td>
            ...
            <button type="button" class="btn '.$btnedit.' btn-sm rounded-pill mt-2" data-toggle="modal" data-target="'.$targetmodal.'" data-idini="' . $data['id_initiative'] . '" data-idkr="' . $data['id_kr'] . '" data-valini="' . $data['value_initiative'] . '" data-valach="' . $data['value_ach_initiative'] . '" data-desc="' . $data['description'] . '" data-valuepercent="' . $data['value_percent'] . '" data-startdate="' . date("Y-m-d",strtotime($data['start_dateinit'])) . '" data-duedate="' . date("Y-m-d",strtotime($data['due_dateinit'])) . '">
              <span class="btn-inner--text">Edit</span>
            </button>
            <a href="" class="btn '.$btndel.' btn-sm rounded-pill mt-2 '.$class.'" data-target="' . base_url() .'project/deleteInisiative/' . $id_project .'/'. $id_okr .'/'.$data['id_kr'] .'/'.$data['id_initiative'] .'" >
            <span class="btn-inner--text">Hapus</span>
           </a>
            </td>
            <td>
              <a type="button" class="btn btn-default btn-sm rounded-pill mt-2 text-white tooltip-custom" data-toggle="modal" data-target="#docIniModal" data-idini="' . $data['id_initiative'] . '" data-idokr="' . $id_okr . '" data-idpj="' . $id_project . '">
              <span class="btn--inner-icon">
              <i class="ni ni-archive-2"></i></span>
              <span class="btn-inner--text"></span>
            </a>
            <a type="button" class="btn btn-primary btn-sm rounded-pill mt-2 text-white" data-toggle="modal" data-target="#taskIniModal" data-namaini="' . $data['description'] . '" data-idini="' . $data['id_initiative'] . '" data-idokr="' . $id_okr . '" data-idpj="' . $id_project . '">
              <span class="btn--inner-icon">
                                            ...
                                            <table class="table align-items-center">
                                              <thead class="thead-light">
                                                <tr>
                                                  ...
                                                  <th scope="col" class="sort" data-sort="name">Start > Due</th>
    ...
    public function valueProjectIni($id_project, $id_okr,$idkr,$iduser)
    {
      ...
      $this->db->set('value_project', $value_project);
      $this->db->set('updated_project', $date);
      $this->db->where('id_project', $id_project);
      $this->db->update('project');
    ...
    public function editInitiative() {
      ...
      $startdate  = $this->input->post('datestarteditini');
      $enddate    = $this->input->post('dateendeditini');
      ...
      $this->db->set('start_dateinit', $startdate);
      $this->db->set('due_dateinit', $enddate);
      $this->db->where('id_initiative', $idini);
      $this->db->update('initiative');
...
$spreadsheet->getActiveSheet()
  ...
  $spreadsheet->getActiveSheet()
    ...
    public function delegateKr($id)
    {
      ...
      $action       = $this->input->post('action');
      $idtimdelegate       = $this->input->post('idtimdelegate');
      ...
      if($action == "delegasikan") {
        for ($i = 0; $i < $ids; $i++) {
          $datarise = array(
            'id_kr_delegate'    => $kr['id_kr'],
            'id_tim_delegate'   => $idtimdelegate,
            'id_user_delegate'  => $id_user[$i],
            'status_delegate'   => '2',
          );
    ...
    public function getTeamMembers($idtim) {
      $spaceteam = $this->Team_model->getTeamProjectAllOkr($idtim);
  
      echo json_encode($spaceteam);
  }

    public function selectAnggotaOKR()
    {
        ...
        $this->db->set('can_edit_okr', $can_edit);
        $this->db->set('can_delete_okr', $can_delete);
        $this->db->where('id_access_team', $idteam);
    ...
    public function hapusAnggotaOKR() {
      ...
      $this->Project_model->hapus_team($id);
  
      // Set flash message dan redirect
      echo json_encode(['success' => true]);
  }

  public function tambahAnggotaOKR()
  {
      $idteam = $this->input->post("idteam");
      $usersend = $this->input->post("usersend");
      $statususer = $this->input->post("tambahstatususer");

      // print_r($usersend);exit();
      ...
      foreach ($usersend as $iduser) {
          $data = array(
              'id_team'           => $idteam,
              'id_user'           => $iduser,
              'role_user'         => $statususer,
              'can_edit_okr'      => $can_edit,
              'can_delete_okr'    => $can_delete,
          );

        
        
      
          $this->db->insert('access_team', $data);
  ...
  public function upload_image() {
        // Get the current year and month to create the folder
        $yearMonth = date('Ym'); // Format: YYYYMM, for example 202409
        
        // Set the upload path to assets/keyresultimg/{yearMonth}
        $uploadPath = './assets/krimg/' . $yearMonth;

        // Check if the folder already exists, if not create it
        if (!is_dir($uploadPath)) {
          if (!mkdir($uploadPath, 0777, true)) {
              // Tambahkan pesan kesalahan jika mkdir gagal
        ...
        $config['max_size'] = 10048; // Maximum size in KB (2MB)
        $config['file_name'] = time(); // Save the file with a unique name (timestamp)

        // Load upload library with the defined configuration
        $this->load->library('upload', $config);

        // Check if the upload process was successful
        if (!$this->upload->do_upload('image')) {
            // If upload fails, return the error message
            $response = array('status' => 'error', 'message' => $this->upload->display_errors());
            echo json_encode($response);
        } else {
            // If upload succeeds, get the file data and return the file URL
            $uploadData = $this->upload->data();
            $filePath = base_url('assets/krimg/' . $yearMonth . '/' . $uploadData['file_name']);

            // Path file yang diupload
            $fullFilePath = $uploadPath . '/' . $uploadData['file_name'];

            // Cek ukuran file setelah diupload, jika lebih dari 2MB (2048 KB), maka kompres gambar
            if ($uploadData['file_size'] > 10048) { // 2048 KB = 2 MB
                $this->compress_image($fullFilePath, $uploadData['file_type']);
            }

            $response = array('status' => 'success', 'imageUrl' => $filePath);
            echo json_encode($response);
        }
    }

      // Fungsi untuk mengkompres gambar
      private function compress_image($filePath, $fileType) {
        // Tentukan kualitas kompresi (0 = paling buruk, 100 = terbaik)
        $quality = 75;

        // Cek tipe file dan gunakan fungsi GD yang sesuai
        switch ($fileType) {
            case 'image/jpeg':
            case 'image/jpg':
                // Load gambar JPEG
                $image = imagecreatefromjpeg($filePath);
                // Kompres dan simpan gambar ke path yang sama
            ...
            case 'image/png':
                ...
                // Kompres PNG (0 = tidak ada kompresi, 9 = kompresi maksimal)
                $pngQuality = 9 - floor($quality / 10);
                imagepng($image, $filePath, $pngQuality);
                break;

            case 'image/gif':
                // GIF tidak dikompres, tapi kita bisa menyimpannya kembali untuk memastikan
                $image = imagecreatefromgif($filePath);
                imagegif($image, $filePath);
                break;

            default:
                // Jika format tidak didukung, kembalikan
                return false;
        }
